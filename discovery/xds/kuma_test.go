...
func mustSerializeKumaMadsV1Resources(resources ...*MonitoringAssignment) []*any.Any {
	serialized := make([]*any.Any, len(resources))
	for i, res := range resources {
		data := mustJSONMarshal(res)
		serialized[i] = &any.Any{
...
func TestKumaMadsV1ResourceParserValidResources(t *testing.T) {
	resources := mustSerializeKumaMadsV1Resources(testKumaMadsV1Resources...)
	groups, err := kumaMadsV1ResourceParser(resources, KumaMadsV1ResourceTypeURL)
	require.NoError(t, err)
	...
	expectedGroup1 := &targetgroup.Group{
		Targets: []model.LabelSet{
			{
				...
				"__meta_kuma_commit_hash": "620506a88",
			...
			},
			{
				...
				"__meta_kuma_commit_hash": "3513bba00",
		...
		Labels: model.LabelSet{
			...
			"__meta_kuma_team":    "infra",
			"__meta_kuma_zone":    "us-east-1",
		},
	}
...
func TestKumaMadsV1ResourceParserInvalidResources(t *testing.T) {
	resources := []*any.Any{{
		TypeUrl: KumaMadsV1ResourceTypeURL,
		Value:   []byte("not-a-protobuf-value"),
	}}
	groups, err := kumaMadsV1ResourceParser(resources, KumaMadsV1ResourceTypeURL)
	require.Nil(t, groups)
	require.Error(t, err)
...
func TestNewKumaHTTPDiscovery(t *testing.T) {
	...
	require.Equal(t, kumaConf.ClientName, resClient.config.ClientID)
...
func TestKumaHTTPDiscoveryRefresh(t *testing.T) {
	s := createTestHTTPServer(t, func(request *v3.DiscoveryRequest) (*v3.DiscoveryResponse, error) {
		...
		for _, madsRes := range testKumaMadsV1Resources {
			resources = append(resources, &any.Any{
				TypeUrl: request.TypeUrl,
				Value:   mustJSONMarshal(madsRes),
			})
		}
